...
* assuming BR doesn't have a fixed point just because strategy sets are hard to compare (e,g., categories and numbers)
* assuming BR has a fixed point just because the same value pops up in everyone's individual BR correspondence--YOU HAVE TO CHECK THE BOUNDARIES!!
* starting with too many cases when deriving a BR correspondence - and even worse: not checking enough cases after coming up with a few initial cases. e.g., if $p_{-i} < v$ is a case, don't forget to check negative values, $0$, or even just small values -- even if there's not a clear "cut-line" like 0, it's possible that you need another case at $p_{-i} \leq v/2$.  


### "Algorithm Find all PSNE - infinite"

1. Write down $BR_i, \forall i$
    a. just think through it and do a bunch of cases, splitting $s_{-i}$ at natural split points (e.g., 0)
    b. checking everything twice and try with both more and fewer cases after you get the first version of the BR correspondence to check that everything is right
2. Say "PSNE $\iff$ fixed point of $BR = \Pi_i BR_i$"
3. Look for fixed points or mutual best responses
    a. if all are numeric, then great. look for matching numbers and then ALWAYS check the limits over which those numbers are a BR.
    b. if fixed point hard to see (or doesn't exist which can sorta happen if some strats are infinite and others are categorical), pick one player, go case-by-case through her BR correspondence and ask whether all players have a best response to that best response


> don't be afraid to make a table if there are too many cases! it ended up being pretty quick to do 18 cases this way!
...
2. Break into cases in terms of what $-i$ does
    ...
    b. try starting with fewer cases and then adding additional cases to check the fewer cases
    c. 0 is usually a nice starter case
...
4. Say, "I've checked all possible cases and player $i$ always wants to deviate so no PSNE!"
...
1. Look for strategies that are best responses to the simplest beliefs.
2. Look for strategies that are dominated by other pure strategies
