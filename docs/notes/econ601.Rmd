...
1. $\succsim$ rational $\Rightarrow \succ$ asymmetric and transitive
2. $\succsim$ rational $\Rightarrow \sim$ symmetric and transitive
3. $\succsim$ rational $\Rightarrow x \succsim y \succsim z$ and either $x\not\sim y$ or $y \not\sim z$ implies $x\succ z$
...
1. $\succsim$ is risk-averse / `r colorize("-seeking", "red")` / `r colorize("-neutral", "green")`
2. $[p$ is a simple MPS of $q] \Rightarrow q \succsim p$
3. $u$ is concave / `r colorize("convex", "red")` / `r colorize("affine", "green")`
...
Constant Relative Risk Aversion (CRRA) means risk attitude does not depend on inflation.
...
5. $\frac{\partial}{\partial p_i} e(p,u) = x^h_i(p,u), \forall i = 1, ..., n.$
    * This is Shephard's Lemma. It's the counterpart to Roy's identity, just on the Hickisan side (and simpler).
...
> $i,k$ can be same (sensitivity of demand to price of own good) or different (sensitivity of demand to price of another good)
...
### "Typical 601 questions"
1. Choice/ Decision theory
    * silly transitivity/completeness proofs
    * WARP and choice structures (proofs)
2. Utility
    * what amount of wealth to make you indifferent
    * what is expected value of bet
    * is $U$ neoclassical?
3. Demand theory
    * write down indirect utility function
    * derive demand / use duality
...
### Algorithm "Does this choice function satisfy WARP?"

1. Assume violation
    i. try minimal counterexample (3-4 elements, + WARP def)
    ii. can try to violate just $\alpha$ or just $\beta$
2. No violation
    i. Choose appropriate WARP def
    ii. define $A, B, c(A), c(B), x, y, ...$
    iii. prove, usually with cases
    
### Algorithm "expected utility questions"

WHAT QUESTIONS ARE TYPICAL HERE??
